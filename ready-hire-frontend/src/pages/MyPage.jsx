    ...
    <>
      {loading ? (
        ...
        <>
          ...
          <Section title="면접 히스토리">
            {history.length === 0 ? (
              <p className="text-sm text-gray-500">아직 면접 기록이 없습니다.</p>
            ) : (
              <ul className="space-y-3">
                {history.map((item) => (
                  <li key={item.interviewId} className="rounded-2xl border border-gray-100 p-4">
                    <button
                      type="button"
          ...
          <Card as="section" className="border-red-100">
            <h2 className="text-lg font-semibold text-red-700">회원 탈퇴</h2>
            <p className="mt-2 text-sm text-gray-600">
              탈퇴 시 계정이 비활성화되며 PRO 구독이 해지됩니다. 면접 기록은 보관되지만 다시 로그인할 수 없습니다.
            </p>
            <Button variant="dangerOutline" size="sm" onClick={() => setWithdrawOpen(true)} className="mt-4">
              회원 탈퇴
            </Button>
          </Card>
        </>
